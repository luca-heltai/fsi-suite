...
namespace dealii::NonMatching
{
   ...
   *        $$M_{ij}:= \int_B v_i w_j dx$$ as products of cellwise smooth
            functions on the intersection of the two grids. This information is
            described by a `std::vector<std::tuple>>` where each tuple contains
            the two intersected cells and a Quadrature formula on their
   intersection.
   *
