...
namespace PDEs
{
  namespace MPI
  {
    ...
    Stokes<dim>::Stokes()
      ...
      , ilu_preconditioner("/Stokes/Solver/ILU preconditioner")
      , velocity(0)
      , pressure(dim)
    {
    ...
    Stokes<dim>::solve()
    {
      ...
      ilu_preconditioner.initialize(m.block(1, 1));

      auto precA = linear_operator<Vec>(A, this->preconditioner);
      auto precS = -1.0 * linear_operator<Vec>(Mp, ilu_preconditioner);
