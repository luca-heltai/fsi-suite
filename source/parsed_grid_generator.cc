...
template <int dim, int spacedim>
void
ParsedGridGenerator<dim, spacedim>::generate(
  dealii::Triangulation<dim, spacedim> &tria) const
{
  ...
  if (transform_to_simplex_grid == true &&
      tria.all_reference_cells_are_hyper_cube() == true)
    {
      ...
      tria.clear();
      GridGenerator::convert_hypercube_to_simplex_mesh(tmp, tria);
  ...
  //     // now take care of additional manifolds using grid_generator_arguments
  // #ifdef DEAL_II_WITH_OPENCASCADE
